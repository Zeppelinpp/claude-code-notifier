...
This skill configures your Bark device key so that CodePing can send push notifications to your iPhone when Codex, Claude Code, or Kimi stops responding.
...
2. Writes it to `~/.codex/plugin-configs/codeping/bark-key` and `~/.claude/plugin-configs/claude-code-notifier/bark-key`
...
3. Create the config directories if they don't exist
4. Write the key to both `~/.codex/plugin-configs/codeping/bark-key` and `~/.claude/plugin-configs/claude-code-notifier/bark-key` with permissions 600
5. Confirm success and mention they can test it by triggering a Codex or Claude Code Stop event
